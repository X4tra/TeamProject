#!/usr/bin/env python3
"""
detect_pygame.py — Vehicle detector + zone counter for Raspberry Pi
====================================================================

No ultralytics, no torch required. Uses NCNN directly.

Display backend:
  - pygame (reliable on Raspberry Pi when cv2.imshow is flaky or unavailable)
...
Usage examples
--------------
  python3 detect_pygame.py                                   # Pi camera, fullscreen
  python3 detect_pygame.py --source 0                       # USB/CSI cam via OpenCV
  python3 detect_pygame.py --source finalvehicle.mp4        # prerecorded video
  python3 detect_pygame.py --no-fullscreen                  # windowed mode
  python3 detect_pygame.py --no-flip                        # disable horizontal flip
  python3 detect_pygame.py --frame-skip 3 --conf 0.35      # tuning options
"""
...
class PygameDisplay:
    def __init__(self, width, height, fullscreen=True, title=WINDOW_NAME):
        if pygame is None:
            sys.exit(
        ...
        # Helps SDL pick the right display on Raspberry Pi desktop sessions.
    ...
    def show(self, frame_bgr):
        ...
        # OpenCV uses BGR, pygame expects RGB.
        frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)

        # pygame.surfarray.make_surface expects array shape: (width, height, 3)
        frame_rgb = np.ascontiguousarray(frame_rgb.swapaxes(0, 1))
...
def run(args):
    ...
    display = PygameDisplay(DISPLAY_W, DISPLAY_H, fullscreen=args.fullscreen)
